...
cnt = 67
for page in range(1, 18+1): # 이미지 페이지 수 
    print('page : '+ str(page))
    # 이미지 홈페이지 주소 
    url = 'https://www.forhair.co.kr/gallery/before_after.asp?branch=&lossType=&page='+str(page)+'#'
    ...
    for img_number in range(1,5+1): # 페이지의 이미지 장수
        # 이미지 주소리스트 
        img_url =[] 
        # 텍스트 리스트 
        info_text= [] 
        ...
        try :
            before_PATH = '//*[@id="compareView"]/div['+str(img_number)+']/div[1]/img'
            img_url.append(url_print(before_PATH))
            after_PATH= '//*[@id="compareView"]/div['+str(img_number)+']/div[2]/img'
        ...
        for idx,url in enumerate(img_url):
            urlretrieve(url, f'./downloads/forhair/'+str(cnt)+'-'+ str(idx+1) +'.jpg')
        cnt += 1
