...
cnt = 2000
for page in range(19, 30+1): # 이미지 페이지 수 
    print('page : '+ str(page))
    # 이미지 홈페이지 주소 
    url = 'http://moandlineps.com/case.html?page_num='+str(page)+'&bid=case&cate=all'
    browser.get(url)
    ...
    for img_number in range(1,10+1): # 페이지의 이미지 장수
        # 이미지 주소리스트 
        img_url =[] 
        # 텍스트 리스트 
        info_text= [] 
        ...
        try :
            before_PATH = '//*[@id="container"]/div/div[3]/ul/li['+str(img_number)+']/a/div[1]/div[1]/div[1]/img'
                         # //*[@id="container"]/div/div[3]/ul/li[8]/a/div[1]/div[1]/div[1]/img
            img_url.append(url_print(before_PATH))
            after_PATH= '//*[@id="container"]/div/div[3]/ul/li['+str(img_number)+']/a/div[1]/div[1]/div[3]/img'
            img_url.append(url_print(after_PATH))
            before_PATH = '//*[@id="container"]/div/div[3]/ul/li['+str(img_number)+']/a/div[1]/div[2]/div[1]/img'
            img_url.append(url_print(before_PATH))
            after_PATH= '//*[@id="container"]/div/div[3]/ul/li['+str(img_number)+']/a/div[1]/div[2]/div[3]/img'
            img_url.append(url_print(after_PATH))
        ...
        for idx,url in enumerate(img_url):
            urlretrieve(url, f'./downloads/moandline/'+str(cnt)+'-'+ str(idx+1) +'.jpg')
        cnt += 1
